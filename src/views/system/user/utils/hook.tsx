...
export function useUser(tableRef: Ref, treeRef: Ref) {
  ...
  const buttonClass = computed(() => [
    "h-5!", "reset-margin", "text-gray-500!",
  ...
  async function onbatchDel() {
    ...
    await ElMessageBox.confirm(`確認刪除用戶 ID：${ids.join(", ")} ？`, "警告", { type: "warning" });
    let success = true;
    for (const id of ids) {
      const res = await deleteUser(id);
      if (res.code !== 0) { success = false; break; }
    }
    if (success) {
      message("批量刪除成功", { type: "success" });
      tableRef.value.getTableRef().clearSelection();
      onSearch();
    }
  }
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(async valid => {
          ...
          if (title === "新增") {
            ...
            if (res.code === 0) {
              message(`新增用戶「${curData.username}」成功`, { type: "success" });
              done();
              onSearch();
            }
          } else {
            const res = await updateUser({ ...curData, id: row?.id });
            if (res.code === 0) {
              message(`修改用戶「${curData.username}」成功`, { type: "success" });
              done();
              onSearch();
            }
          }
  ...
  async function handleDelete(row) {
    await ElMessageBox.confirm(`確認刪除用戶「${row.username}」？`, "警告", { type: "warning" });
    const res = await deleteUser(row.id);
    if (res.code === 0) {
      message(`已刪除用戶「${row.username}」`, { type: "success" });
      onSearch();
    }
  }
    ...
    addDialog({
            ...
            {pwdProgress.map(({ color, text }, idx) => (
              <div class="w-[19vw]" style={{ marginLeft: idx !== 0 ? "4px" : 0 }}>
                <ElProgress
                  ...
                  color={color} stroke-width={10} show-text={false}
                />
      ...
      beforeSure: async done => {
        ruleFormRef.value.validate(async valid => {
          if (!valid) return;
          const res = await updateUser({ id: row.id, password: pwdForm.newPwd });
          if (res.code === 0) {
            message(`已成功重置 ${row.username} 用戶密碼`, { type: "success" });
            done();
          }
        });
      }
  ...
  async function handleRole(row) {
    ...
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        console.log("roleIds", curData.ids);
        done();
      }
    });
  }
  ...
  // tree (dept removed, keep for compat)
  function onTreeSelect({ id, selected }: any) {}
